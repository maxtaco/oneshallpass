...
the input: security bits, password length, generation, symbols needed, and
notes.  Note that your plaintext passphrase is never sent to the server.
...
a deterministic IV for the key so that differing browsers will always come up
...
server, and used to populate the given dropdown field.
